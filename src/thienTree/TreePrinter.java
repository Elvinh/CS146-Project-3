...
public class TreePrinter 
{
    ...
    private tBST<Integer> tree;  // the tree
     ...
     */
    public TreePrinter(tBST<Integer> tree)
    {
        this.tree = tree;
        this.height = tree.showHeight();
    }
    ...
    {       
        ...
        t_sTreeNode<Integer> thisLevelNodes[] = 
                (t_sTreeNode<Integer>[]) new t_sTreeNode[1];
        
        int offset = POWERS_OF_2[(height+1)]-1;
        
        thisLevelNodes[0] = tree.tRoot;
    ...
    private void printData(int level, int offset, 
                           t_sTreeNode<Integer> levelNodes[])
    {
        printSpaces(offset);
        
        int k = POWERS_OF_2[level];
        for (int i = 0; i < k; i++) {
            t_sTreeNode<Integer> node = levelNodes[i];
                                       ...
                                       t_sTreeNode<Integer> levelNodes[])
    {
        ...
        for (int i = 0; i < k; i++) {
            ...
            if ((node != null) && (node.lftChild != null)) {
                System.out.print(" /");
            }
            ...
            if ((node != null) && (node.rtChild != null)) {
                System.out.print("\\ ");
            }
     ...
     */
    private t_sTreeNode<Integer>[] nextLevel(int level, 
                                            t_sTreeNode<Integer> levelNodes[])
    {
        t_sTreeNode<Integer> nextLevel[] = 
                (t_sTreeNode<Integer>[]) new t_sTreeNode[POWERS_OF_2[level+1]];
        
        for (int i = 0; i < POWERS_OF_2[level]; i++) {
            t_sTreeNode<Integer> node = levelNodes[i];
            
            // Queue the left child nodes of each non-null parent node.
            nextLevel[2*i] = (node != null) && (node.lftChild != null)
                           ? node.lftChild : null;
                                        
            // Queue the right child nodes of each non-null parent node.
            nextLevel[2*i+1] = (node != null) && (node.rtChild != null)
                             ? node.rtChild : null;
        }
